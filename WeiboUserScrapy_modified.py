...
class WeiboUserScrapy():
    ...
    def __init__(self, user_id, filter=0, download_img=False, start_date=None, end_date=None):
        global headers
        self.headers = {
            'Cookie': Cookie,
            ...
            if is_exist:
                for i in range(0, len(info) - 1):
                    ...
                    if weibo:
                        ...
                        publish_time_str = weibo['publish_time']
                        publish_time = datetime.strptime(publish_time_str, '%Y-%m-%d %H:%M')

                        if publish_time < self.start_date:
                            self.write_file(0)
                            self.stop = True
                            return
...
def process_accounts(input_dir='input_csv'):
    """
    处理不同类别的单行CSV文件
    自动在您函数生成的文件名前添加类别前缀
    """
    for filename in os.listdir(input_dir):
        if filename.endswith('.csv'):
            category = os.path.splitext(filename)[0]  # 获取类别名
            input_path = os.path.join(input_dir, filename)
            
            print(f"\n处理类别: {category}")
            
            with open(input_path, 'r', encoding='utf-8') as infile:
                accounts = next(csv.reader(infile))  # 读取单行CSV
                
                for account in accounts:
                    account = account.strip()
                    if account:
                        WeiboUserScrapy(user_id = int(account), filter = 0, download_img = False, start_date = '2024-08-01', end_date = '2025-11-14')
...
if __name__ == '__main__':
    ...
    # 3、可选下载所有图片（包括微博相册和转发微博里面的图片），参数为 download_img，默认为 False 不下载
        if not os.path.exists('input_csv'):
            print("请创建 'input_csv' 目录并放入分类CSV文件")
        else:
            process_accounts()
            print("\n所有账号处理完成")